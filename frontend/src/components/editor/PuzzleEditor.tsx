...
import type { PuzzleDef } from "../../types/story";
...
export function PuzzleEditor({
  ...
  const puzzleIds = Object.keys(puzzles).sort();
  const puzzle = selected ? puzzles[selected] : null;

  const handleSave = async (puzzleId: string, data: PuzzleDef) => {
    try {
      const { id: _, ...rest } = data;
      await storyApi.upsertPuzzle(storyId, puzzleId, rest);
      showStatus(`Puzzle "${puzzleId}" saved`);
      onReload();
    } catch (err) {
  ...
  const handleDelete = async (puzzleId: string) => {
    if (!confirm(`Delete puzzle "${puzzleId}"?`)) return;
    await storyApi.deletePuzzle(storyId, puzzleId);
    setSelected(null);
    showStatus(`Puzzle "${puzzleId}" deleted`);
    onReload();
  };
  ...
  const handleCreate = async () => {
    ...
    const id = newId.trim();
    await storyApi.upsertPuzzle(storyId, id, {
      name: id,
      description: "",
      steps: [],
      timed_window: null,
      ...
      <div className="flex-1 min-w-0">
        {puzzle && selected ? (
          <div className="space-y-4">
            ...
            <JsonEditor
              data={puzzle}
              onSave={(data) => handleSave(selected, data as PuzzleDef)}
            />
          </div>
        ) : (
